...
- Per ogni riga y di W, calcola il prodotto scalare con x: $W_y \cdot x = \sum_i W_{yi} x_i = f_y$

- Applica la funzione softmax per ottenere la probabilità normalizzata: $p(y|x) = \frac{\exp(f_y)}{\sum_c \exp(f_c)} = \text{softmax}(f_y)$

- Scegli la y con la probabilità massima.
