...
*L'obiettivo è di assegnare i record non noti a una classe nel modo più accurato possibile.* Per farlo, viene utilizzato il training set per costruire il modello e il test set per validarlo.

I classificatori si dividono in classificatori di base e classificatori Ensemble *(Boosting, bagging, random forest).*
...
* **Nodi interni o di partizionamento:** rappresentano gli *attributi di splitting* sui quali vengono poste le domande.
...
* La *procedura di costruzione* di un albero decisionale a partire da un training set Dt con possibili etichette di classe yt = y1, ..., yk è la seguente:
- Se Dt contiene record di una sola classe yj, il nodo t diventa una foglia con etichetta yj.
- Altrimenti, si sceglie un attributo e un criterio per suddividere Dt in sottoinsiemi non vuoti.
- Si applica ricorsivamente la stessa procedura sui sottoinsiemi ottenuti.
...
* *p(j|t) rappresenta la frequenza delle istanze del training set della classe j nel nodo t*
...
*Il guadagno è una misura non negativa, in quanto P>M.*
...
*dove nc rappresenta il numero di classi*
...
*Per ogni coppia di valori contigui ne scelgo uno intermedio. In base ai valori ottenuti provo i partizionamenti e scelgo quello con l'indice di gini minore.*

### Entropia

$$Entropy(t)=- \sum_{j=1}^k p(j|t)\log_{2}(p(j|t))$$

*dove t rappresenta il nodo e p(j|t) è la frequenza relativa alla classe j nel nodo t*
...
* Incorporare la complessità del modello nell'errore *(Occam's Razor: preferire il modello più semplice).*
...
- Ω: parametro che stima la penalità dell’aggiunta di un nodo foglia *(modula la penalizzazione).*
...
* **Minium Description Lenght**
	* Minimizza il costo per descrivere una classificazione. Il costo totale è dato dalla *somma del costo per codificare gli errori di classificazione del modello sui dati e del costo per codificare la complessità stessa del modello.*
...
*Dove il primo termine è il costo degli errori di classificazione e il secondo è il costo di codifica della complessità dell'albero, pesato da un fattore α.*
...
* **Vantaggio:** *evita di generare sottoalberi eccessivamente complessi che potrebbero portare a overfitting.*
* **Svantaggio:** talvolta *espansioni successive potrebbero portare a migliori sottoalberi, che però non vengono raggiunti a causa dell'arresto prematuro.*
...
* **Vantaggio:** *le decisioni di potatura si basano su un albero inizialmente completo, tendendo a restituire risultati migliori.*
* **Svantaggio:** *maggiore costo computazionale dovuto alla necessità di sviluppare inizialmente l'albero completo.*
...
* **Cross Validation:**
	...
	* *Il dataset di dimensione N viene partizionato in k sottoinsiemi distinti di dimensioni uguali. Un sottoinsieme viene usato come test set, mentre i rimanenti k-1 sottoinsiemi formano il training set per addestrare il modello di classificazione. Questo processo viene ripetuto k volte, con k modelli diversi addestrati e validati.*
...
* **Bootstrap:**
	* Si effettua un ricampionamento con reinserimento (reimbussolamento) dei record già selezionati per costruire il training set. *Ogni record ha la stessa probabilità di essere estratto nuovamente.*
...
In molti dataset, le classi sono *distorte,* ossia vi sono molti più record di una classe rispetto alle altre. Ciò pone due sfide per la classificazione:
...
* *Nel caso di classificazione binaria, la classe rara è chiamata anche classe positiva, mentre quella che include la maggior parte dei record è chiamata classe negativa.*
...
* Queste tecniche utilizzano una Matrice dei Costi che, a differenza della Confusion Matrix, introduce dei *parametri di penalità* per indicare il costo di classificare erroneamente un record in una classe sbagliata.
...
* *Soluzione 1:* Definire un ordine di attivazione delle regole.
* *Soluzione 2:* Assegnare il record alla classe per la quale vengono attivate più regole.
...
* *Copertura pari a zero e si assume una distribuzione uniforme dei dati ->* l'indice si riduce alla probabilità a priori della classe.
* *Copertura alta ->* l'indice tende asintoticamente al valore dell'accuratezza.
* *Copertura bassa ->* l'indice tende a diminuire.
...
* Il guadagno di informazioni di FOIL della regola estesa è calcolato in base *alla differenza tra i logaritmi delle proporzioni di esempi positivi nella regola estesa e nella regola iniziale.*
...
Semplifica le regole di *learn one rule* per migliorare l'errore di generalizzazione delle regole stesse. Ѐ utile perché l'approccio di costruzione è greedy.
...
* Se non viola la *Minimum description length principle* (condizione di stop), la regola viene aggiunta al set di regole.
...
- Le regole di classificazione vengono estratte per ogni percorso dalla radice ai nodi foglia dell'albero decisionale.
- Per ogni regola `r : A → y`:
...
- Le regole caratterizzate dal medesimo conseguente (classe) vengono raggruppate all'interno dello stesso sottoinsieme.
- Viene calcolato per ciascun sottoinsieme il valore di *description length* così definito:
...
* **Classificazione:**
- Calcola la vicinanza di un'istanza di test alle istanze del training set utilizzando una misura di prossimità.
- Identifica i k nearest neighbors (i k record più vicini).
- Determina la classe del record sconosciuto scegliendo la classe più frequente tra i k vicini.
...
- Inserisci la radice in uno stack.
- Estrai un nodo T dallo stack.
...
- **Selezione della dimensione:** Si sceglie una dimensione (asse) lungo la quale partizionare l'insieme dei punti. La scelta può essere fatta ciclicamente tra le dimensioni.
- **Calcolo del valore mediano:** Si calcola il valore mediano dei punti lungo la dimensione scelta e si crea un nodo corrispondente a quell'iper-piano di separazione.
- **Divisione dei punti:** Si dividono i punti in due sottoinsiemi: quelli a sinistra (minori) e quelli a destra (maggiori) del piano di separazione lungo la dimensione scelta.
- **Ricorsione:** Il processo viene ripetuto ricorsivamente sui due sottoinsiemi, selezionando una nuova dimensione di separazione, fino a che ogni nodo contiene al massimo un punto oppure si raggiunge una dimensione massima dell'albero.

### Ricerca in un Kd-tree

- **Partenza dalla radice:** Si parte dalla radice dell'albero.
- **Discesa ricorsiva:** Si scende ricorsivamente verso il basso seguendo il ramo sinistro o destro in base al lato del piano di separazione del nodo corrente in cui cade il punto di query.
- **Nodo foglia:** Si arriva ad un nodo foglia contenente un singolo punto, che diventa il "miglior punto trovato finora".
- **Risalita ricorsiva:** Risalendo ricorsivamente, per ogni nodo si controlla se il suo sottoalbero opposto a quello appena esplorato potrebbe contenere un punto più vicino al punto di query.
- **Controllo della distanza:** Se la distanza del punto di query dal piano di separazione è minore della distanza dal miglior punto trovato, si esplora anche l'altro sottoalbero.
- **Continuazione della risalita:** Altrimenti si continua a risalire senza esplorare ulteriormente quella porzione.
...
- Partizionare gli oggetti in gruppi di piccola cardinalità (2-3 elementi).
- Per ogni gruppo, calcolare il rettangolo minimo che racchiude tutti i suoi oggetti.
- Unire ricorsivamente i rettangoli minimi in nodi intermedi, fino ad ottenere un singolo nodo radice che racchiude tutti gli oggetti.
