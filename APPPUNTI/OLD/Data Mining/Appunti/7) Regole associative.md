...
dove *N* è il numero totale di oggetti.

Un criterio importante per la valutazione delle regole è che la confidenza della regola *c(X → Y)* sia maggiore del supporto di *Y* (s_Y). Questo significa che la regola è più informativa rispetto a semplicemente sapere che *Y* è presente nel dataset.

La tabella di contingenza fornisce un quadro completo per analizzare le relazioni tra *X* e *Y* e per valutare l'utilità della regola *X → Y*

## Indipendenza Statistica

La condizione per una regola associativa *X → Y* di essere interessante è che la probabilità condizionata di *Y* dato *X* sia maggiore della probabilità di _Y_. Questo può essere espresso come:
...
In questo esempio, la regola con correlazione più alta (0.9) ha un valore di *Lift* inferiore rispetto alla regola con correlazione più bassa. Questo dimostra che il *Lift* non è sempre un indicatore affidabile della forza di una regola.
