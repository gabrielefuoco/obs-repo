...
**Concetto Fondamentale:** Modello ispirato al sistema nervoso umano, composto da nodi (neuroni) interconnessi tramite collegamenti (sinapsi) con pesi associati che rappresentano la forza della connessione.
...
- **Biologico:** Imitazione di sistemi neurali biologici (es. vista, udito). Connettività maggiore ma tempi di commutazione più lenti (ms vs ns) rispetto ai neuroni biologici.
- **Guidato dalle applicazioni:** Architettura condizionata dalle esigenze applicative.

**Perceptron (Rosenblatt, 1962):** Rete neurale semplice a singolo strato.
...
**Funzione di attivazione:** `sign()` (non lineare, fondamentale per evitare una semplice funzione lineare).

**Addestramento:** I pesi $w_i$ e il bias $b$ vengono addestrati per risolvere problemi di classificazione binaria.
...
Inizializzazione:
- Definizione del dataset di training: $D_{train} = \{(x_k, y_k) | k = 1, 2, ..., n\}$
- Inizializzazione dei pesi e bias: $(w^{(0)}, b^{(0)})$ con valori randomici.
- Inizializzazione del contatore di iterazioni: $c = 0$.

Iterazione finché convergenza: $(w^{(c+1)}, b^{(c+1)}) \approx (w^{(c)}, b^{(c)})$
- Per ogni record $(x_k, y_k) \in D_{train}$:
Calcolo dei valori di attivazione: $(a_i^l)_k$
Calcolo dei valori di errore: $(\delta_i^l)_k$ (backpropagation)
Calcolo della perdita e della sua derivata: $(Loss)_k, (Loss')_k$
- Calcolo del gradiente rispetto ai pesi: $\frac{\partial E}{\partial w_{ij}^l} = \sum_k (\frac{\partial Loss}{\partial w_{ij}^l})_k$
- Calcolo del gradiente rispetto ai bias: $\frac{\partial E}{\partial b_i^l} = \sum_k (\frac{\partial Loss}{\partial b_i^l})_k$
- Aggiornamento dei pesi e bias tramite discesa del gradiente: $(w^{(c+1)}, b^{(c+1)})$
- Incremento del contatore: $c = c + 1$

##### Caratteristiche delle ANN

Approssimatori Universali (multi-strato)
Problematiche:
- Overfitting
- Minimi locali
- Tempo di costruzione del modello (lento) / test (veloce)
- Gestione attributi ridondanti (vantaggio)

##### Limitazioni delle ANN

Sensibilità al rumore
Gestione attributi mancanti (difficoltà)
