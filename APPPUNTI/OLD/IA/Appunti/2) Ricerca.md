...
- **Espansione dello stato corrente:** Si generano tutti i possibili stati successivi allo stato corrente e si inseriscono in un insieme chiamato **frontiera**.
- **Selezione di un nodo:** Si sceglie un nodo dalla frontiera in base a una strategia specifica.
- **Verifica del goal:** Se il nodo selezionato soddisfa la condizione di obiettivo, la ricerca ha successo e termina.
- **Espansione del nodo:** Se il nodo selezionato non soddisfa il goal, si espande il nodo, ovvero si generano i suoi stati successivi e si aggiungono alla frontiera.
- **Fine della ricerca:** Se la frontiera è vuota, ovvero non ci sono più nodi da espandere, la ricerca fallisce.
...
- **Passaggi logici**:
- Dalla consistenza sappiamo che:
$$
f(p) \leq f(n)
$$
- Inoltre, poiché $n$ e $n'$ si riferiscono allo **stesso stato**, l'euristica deve essere identica per entrambi:
$$
h(n) = h(n')
$$
- Poiché $n$ è sul cammino ottimo e $n'$ è su un cammino non ottimo, sappiamo che $g(n) < g(n')$, ma abbiamo anche che $h(n) = h(n')$. Ne consegue che $f(n) < f(n')$, e dunque:
$$
f(p) \leq f(n) < f(n')
$$
...
- Dall'ipotesi di **Consistenza** sappiamo che:
$$
\forall \text{azione } a, \quad h(n) \leq costo(n, a, goal) + h(goal)
$$
- **Poiché** per definizione $h(goal) = 0$, segue:
$$
h(n) \leq costo(n, a, goal)
$$
- Consideriamo ora l'azione ottima $a*$ che porta al goal con il costo minimo $h(n)$. Allora possiamo scrivere:
$$
...
- Dall'ipotesi di **Consistenza** deriva che:
$$
\forall a, \quad h(n) \leq costo(n, a, n') + h(n')
$$
- Dove $n'$ è un nodo che dista 1 da $n$.

- **Per la migliore azione $a^*$ e il miglior nodo $n'^*$**:
$$
h(n) \leq costo(n, a^*, n'^*) + h(n'^*)
$$

- **Applicando l'ipotesi induttiva**: Poiché l'euristica è ammissibile per gli stati che distano k passi dal goal, come $n'^*$, possiamo utilizzare l'ipotesi induttiva per sostituire $h(n'^*)$ con $h^*(n'^*)$, ovvero il costo effettivo minimo per raggiungere il goal partendo da $n'^*$:
$$
h(n'^*) \leq h^*(n'^*)
$$
