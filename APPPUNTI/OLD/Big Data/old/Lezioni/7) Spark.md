...
- **Immutabilità**: Una volta creati, gli RDD non possono essere modificati. Ogni trasformazione su un RDD crea un nuovo RDD.
- **Distribuzione**: Gli RDD vengono suddivisi in **partizioni**, distribuite tra i nodi del cluster. Ogni partizione rappresenta un’unità di parallelismo.
- **Tolleranza ai guasti**: In caso di guasto, gli RDD possono essere ricostruiti senza replicazione, utilizzando il **lineage** (una traccia della loro creazione).
...
- **Parallelizzando collezioni esistenti** del linguaggio host (come le collezioni o le liste in Scala, Java, Python o R):
- L'utente può specificare il numero di partizioni.
- Nell'API RDD: utilizzando il metodo `parallelize`.
- **Da file di grandi dimensioni** memorizzati in HDFS o in altri file system compatibili:
...
- **Lettura del file** da HDFS come RDD:
   ```scala
   val textFile = sc.textFile("hdfs://...")
   ```

- **Estrazione delle parole** usando `flatMap`:
   ```scala
   val words = textFile.flatMap(line => line.split(" "))
   ```

- **Assegnazione di un conteggio iniziale** di 1 a ogni parola:
   ```scala
   val ones = words.map(word => (word, 1))
   ```

- **Somma delle occorrenze di ogni parola** con `reduceByKey`:
   ```scala
...
- **RDD**: Dataset distribuito e partizionato.
- **DAG**: Grafo logico delle operazioni su RDD.
