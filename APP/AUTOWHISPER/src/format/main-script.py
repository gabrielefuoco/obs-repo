...
def read_prompt(prompt_file_path, subject):
    """Read prompt from file and add university lecture subject."""
    try:
...
def main():
    ...
    parser.add_argument('subject', type=str, help='University lecture subject')
    ...
    prompt = read_prompt(args.prompt_file, args.subject)
    ...
    txt_files = [f for f in os.listdir(args.input_folder) 
                if f.lower().endswith('.md') and f != os.path.basename(args.prompt_file)]
